  ...
  confirm () {
    ...
    try {
      ...
      if (outMessage.length) {
        ...
        if (this.dryCmd) {
          atom.notifications.addSuccess(dirMessage, {detail: outMessage, dismissable: missNote})
        } else {
          const range = editor.getSelectedBufferRange()
          editor.setTextInBufferRange(range, outMessage)
        }
      }
    } catch (e) {
      dirMessage += '\n' + e.stderr.toString()
